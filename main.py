...
features_list = pickle.load(open("clothing1.pkl", "rb"))
img_files_list = pickle.load(open("clothing2.pkl", "rb"))

model = ResNet50(weights="imagenet", include_top=False, input_shape=(224, 224, 3))
model.trainable = False
model = Sequential([model, GlobalMaxPooling2D()])

st.title('Clothing and Acessory recommender system')
...
if uploaded_file is not None:
    if save_file(uploaded_file):
        ...
        st.image(resized_im)
        #st.image(show_images, caption=None, width=None, use_column_width=None, clamp=False, 
